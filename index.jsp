<jsp:include page="includes/head.jsp" />
<title>We the leaders</title>
<link href="assets/css/home.css" rel="stylesheet">
</head>

<body class="home-page">
 
        <section class='hero d-flex align-items-center text-white'>

            <jsp:include page="includes/header.jsp" />

            <div class='container text-center pt-5 pt-lg-0' data-aos='fade-up'>
                <div class="row">
                    <div class="col-12 col-lg-6 text-start">
                        <h1 class='display-4 text-gradient fw-bold'>We the weaver leaders</h1>
                        <p style="font-size: 17px;line-height: 1.4;">Committed to empowering weaving communities, preserving traditional craftsmanship, and creating sustainable opportunities that inspire growth, innovation, and cultural pride.</p>
                        <a href='./our-services.jsp' class='btn btn-warning btn-lg mt-3'>Explore Our Services</a>
                    </div>
                </div>

            </div>
        </section>


        <section class='py-5'>
            <div class='container'>
                <div class='row align-items-center'>
                    <div class='col-lg-4' data-aos='fade-right'>
                        <img src='assets/images/home/founder.jpeg' class='img-fluid rounded shadow'
                            style="aspect-ratio: 2/3;object-fit: cover;">
                    </div>
                    <div class="col-lg-2 d-none d-lg-block"></div>
                    <div class='col-lg-6' data-aos='fade-left'>
                        <img src="./assets/images/logo/weaver-leaders.svg" class="img-fluid mb-3" alt="weaver" style="max-width: 250px;width: 100%;" />
                        <h2 class="display-5 text-gradient fw-bold">About Us</h2>
                        <p>
                            <strong>We the Weaver Leaders</strong>is a community-driven platform committed to preserving and promoting the rich heritage of weaving and textile craftsmanship. Guided by the vision, "Let Us Cherish Weaving, Let Us Foster Weaving," we empower weaving communities while honoring generations of tradition.
                        </p>

                        <p>
                           Through collaboration, innovation, and knowledge sharing, we connect traditional weaving practices with modern opportunities, helping artisans grow through sustainable methods, new skills, and wider market access.
                        </p>
                    </div>
                </div>
            </div> 
        </section> 


            <!-- Vision Section -->
         <section class="py-5">
                <div class="container float-container">
                    <div class="clearfix">
                        <img src="assets/images/our-vision.jpg" alt="Our Vision" data-aos='fade-left'
                            class="img-fluid rounded shadow float-lg-end ms-lg-4 mb-3"
                            style="height: auto;max-width: 600px;width: 100%;">

                        <h2 class=" display-5 text-gradient fw-bold mb-3">Our Vision</h2>

                       <p>
                               We envision a future where weaving is celebrated as both a treasured cultural heritage and a sustainable source of livelihood. By connecting weavers, artisans, designers, and communities, we foster innovation while preserving traditional craftsmanship.

                            <p>
                              Through awareness, collaboration, and sustainable practices, we empower weavers to embrace modern opportunities without losing the authenticity of their craft. Our goal is to create a world where every weaver is valued, every community thrives, and every handcrafted textile is recognized as a symbol of culture, creativity, and excellence.
                            </p>

                    </div>
                </div>
            </section> 




        <!-- Mission Section -->
            <section class="py-5">
                <div class="container">
                    <div class="clearfix">
                        <img src="assets/images/our-mission.jpg" alt="Our Mission" data-aos='fade-right'
                            class="img-fluid rounded shadow float-lg-start me-lg-4 mb-3"
                            style="height: auto;max-width: 600px;width: 100%;">

                        <h2 class="text-gradient display-5 fw-bold mb-3">Our Mission</h2>

                        <p>
                            At <strong>We the Weaver Leaders</strong>, our mission is to empower weavers and strengthen weaving communities through education, skill development, innovation, and sustainable livelihood opportunities. We are committed to preserving traditional textile craftsmanship while helping artisans adapt to modern market demands.
                        </p>

                        <p>Through collaboration, mentorship, market access, and ethical practices, we support artisans in enhancing their skills, expanding opportunities, and achieving long-term growth. Our goal is to build a resilient weaving ecosystem that preserves heritage, promotes innovation, and improves the lives of weavers for generations to come.</p>

                    </div>
                </div>
            </section>

             <!-- What We do Section -->
            <section class="py-5">
                <div class="container text-center">
                  <h2 class="text-gradient display-5 fw-bold mb-3">What We Do</h2>
                        <p class="mb-5 w-75 mx-auto">
                            We the Weaver Leaders works to empower weaving communities, preserve traditional craftsmanship,
                            and create opportunities for sustainable growth through education, collaboration, advocacy,
                            and market development.
                        </p>

                        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-5" data-aos='fade-up'>

                        <div class="col">
                            <div class="service-card">
                                <div class="servicesIcon mb-3"><img src="./assets/images/services/weaver.png" class="img-fluid" alt="" /></div>
                                <div class="servicesHeading">Weaver Empowerment</div>
                                <div class="servicesContent">
                                    Support weavers through training programs, mentorship, capacity building, and
                                    access to resources that enhance their skills and livelihoods.
                                </div>
                                
                            </div>
                        </div>

                        <div class="col">
                            <div class="service-card">
                                <div class="servicesIcon mb-3"><img src="./assets/images/services/skill.png" class="img-fluid" alt="" /></div>
                                <div class="servicesHeading">Skill Development</div>
                                <div class="servicesContent">
                                    Organize workshops, educational initiatives, and technical training to help
                                    artisans improve craftsmanship and adopt modern weaving practices.
                                </div>
                                
                            </div>
                        </div>

                        <div class="col">
                            <div class="service-card">
                                <div class="servicesIcon mb-3"><img src="./assets/images/services/promotion.png" class="img-fluid" alt="" /></div>
                                <div class="servicesHeading">Handloom Promotion</div>
                                <div class="servicesContent">
                                    Promote handloom and handmade textile products by increasing awareness of their
                                    cultural, artistic, and economic value.
                                </div>
                                
                            </div>
                        </div>

                        <div class="col">
                            <div class="service-card">
                                <div class="servicesIcon mb-3"><img src="./assets/images/services/marketing.png" class="img-fluid" alt="" /></div>
                                <div class="servicesHeading">Market Access</div>
                                <div class="servicesContent">
                                    Connect weavers with customers, businesses, exhibitions, and digital platforms
                                    to expand market reach and create sustainable income opportunities.
                                </div>
                                
                            </div>
                        </div>

                        <div class="col">
                            <div class="service-card">
                                <div class="servicesIcon mb-3"><img src="./assets/images/services/collab.png" class="img-fluid" alt="" /></div>
                                <div class="servicesHeading">Community Collaboration</div>
                                <div class="servicesContent">
                                    Foster partnerships among artisans, organizations, educational institutions,
                                    designers, and industry leaders to strengthen the weaving ecosystem.
                                </div>
                                
                            </div>
                        </div>

                        <div class="col">
                            <div class="service-card">
                                <div class="servicesIcon mb-3"><img src="./assets/images/services/policy.png" class="img-fluid" alt="" /></div>
                                <div class="servicesHeading">Advocacy & Recognition</div>
                                <div class="servicesContent">
                                    Advocate for policies, initiatives, and programs that support weavers while
                                    recognizing their invaluable contribution to cultural heritage and society.
                                </div>
                                
                            </div>
                        </div>

                        </div>

                </div>
            </section>


             <!-- CTA Section -->
            <section class="py-5 CTASection">
                <div class="CTABgWrapper">
                    <div class="container">
                        <div class="CTAWrapper text-white text-center w-50 mx-auto">
                            <h2 class="fw-bold mb-3 text-gradient display-5 text-white"> Join our Community</h2>
                            <p>Be a part of our mission to preserve weaving traditions, empower artisans and create opportunities for future generations.</p>
                        </div>
                    </div>
                </div>
                    <div class="container">
                        <div class="CTAFormMain" data-aos='fade-up'>
                            <form class="position-relative">
                              
                            <fieldset>
                                <legend class="text-center text-gradient fw-bold">Register With Us</legend>
                                <p class="text-center">Join our growing community of weavers, supporters, and volunteers.</p>
                               
                                <div class="CTAformField" style="margin-top: 2.5rem;">
                                    <label for="userName" class="form-label">Enter Your Name</label>
                                    <input type="text" id="userName" class="form-control" placeholder="Weaver User">
                                </div>
                                <div class="CTAformField">
                                    <label for="mobNumber" class="form-label">Enter Your Mobile</label>
                                    <input type="text" id="mobNumber" class="form-control" placeholder="9876543210">
                                </div>
                                <div class="CTAformField">
                                    <label for="batchNumber" class="form-label">Enter Your Batch number</label>
                                    <input type="text" id="batchNumber" class="form-control" placeholder="xxxx-xxxx-xxxx">
                                </div>
                                <div class="CTAformField">
                                <label for="userType" class="form-label">User Type</label>
                                <select id="userType" class="form-select">
                                    <option>Viewer</option>
                                    <option>Non-Viewer</option>
                                    <option>Volentier</option>
                                </select>
                                </div>
                                <div class="mt-3">
                                    <button type="submit" class="btn btn-primary w-100">Submit</button>
                                </div>
                               
                            </fieldset>

                            <div class="CTAformInfoWrapper d-none">
                                Application successfully completed. <br/>
                                Your registered Id : 123456789
                            </div>
                            </form>
                        </div>
                   </div>
            </section> 



       <jsp:include page="includes/footer.jsp" /> 
</body>

</html>